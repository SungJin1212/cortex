...
func NewClient(ctx context.Context, cfg Config, hedgedTransport func(rt http.RoundTripper) http.RoundTripper, name string, logger log.Logger, reg prometheus.Registerer) (bucket objstore.InstrumentedBucket, err error) {
	var client objstore.Bucket
	switch cfg.Backend {
	case S3:
		client, err = s3.NewBucketClient(cfg.S3, hedgedTransport, name, logger)
	case GCS:
		client, err = gcs.NewBucketClient(ctx, cfg.GCS, hedgedTransport, name, logger)
	case Azure:
		client, err = azure.NewBucketClient(cfg.Azure, hedgedTransport, name, logger)
	case Swift:
		client, err = swift.NewBucketClient(cfg.Swift, hedgedTransport, name, logger)
	case Filesystem:
